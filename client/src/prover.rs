...
pub fn prove(
    ...
    if let Some(path) = dump_path{
        fs::write(path, serde_json::to_vec(&receipt).expect("Failed to serialize receipt")).expect("Failed to dump receipt");
    }
    receipt
}
...
fn optimize_groth16_proof(){
    ...
    println!("Optimized Receipt size: {:?}", serde_json::to_vec(&optimized_receipt).unwrap().len());
}