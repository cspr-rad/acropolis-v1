...
fn main() {
    ...
    user_public_key.verify(&circuit_inputs.government_public_key, &circuit_inputs.session_signature).expect("Failed to verify session signature");
    government_public_key.verify(&circuit_inputs.user_public_key, &circuit_inputs.public_identity).expect("Failed to verify public identity");
